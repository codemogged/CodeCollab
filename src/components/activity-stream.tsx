...
function CollapsibleCode({ lang, code, lineCount }: { lang: string; code: string; lineCount: number }) {
  ...
  return (
    <div className="my-1.5 overflow-hidden rounded-md border border-black/[0.05] dark:border-white/[0.06]">
      <div
        className="flex cursor-pointer items-center justify-between bg-black/[0.02] px-2.5 py-1 dark:bg-white/[0.03]"
        onClick={() => setCollapsed(!collapsed)}
      >
        <div className="flex items-center gap-1.5">
          <span className="text-[9px] font-semibold uppercase tracking-wider theme-muted">{displayLang}</span>
          <span className="text-[9px] theme-muted">{lineCount} line{lineCount !== 1 ? "s" : ""}</span>
        </div>
        <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
          ...
          <button
            ...
            className="text-[9px] font-medium theme-muted hover:theme-fg transition"
          >
            Copy
          </button>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor"
            className={`h-2.5 w-2.5 theme-muted transition ${collapsed ? "" : "rotate-180"}`}>
            <path fillRule="evenodd" d="M4.22 6.22a.75.75 0 0 1 1.06 0L8 8.94l2.72-2.72a.75.75 0 1 1 1.06 1.06l-3.25 3.25a.75.75 0 0 1-1.06 0L4.22 7.28a.75.75 0 0 1 0-1.06Z" clipRule="evenodd" />
          </svg>
        </div>
      </div>
      {!collapsed && (
        <pre className="overflow-x-auto bg-[#0d1117] px-3 py-2 font-mono text-[10.5px] leading-[1.6] text-green-300/90 dark:bg-[#0a0e14]">
          <code>{code}</code>
        </pre>
      )}
    </div>
